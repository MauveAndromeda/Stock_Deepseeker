...
class OrderRequest(BaseModel):
    ...
    stop_price: float | None = Field(None, gt=0, description="止损价")
...
class TradingAPI:
    ...
    async def get_order(self, order_id: str) -> OrderResponse:
        ...
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Failed to get order: {e!s}"
            )
    ...
    async def get_portfolio(self) -> PortfolioResponse:
        ...
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Failed to get portfolio: {e!s}"
            )
    ...
    async def get_performance(self) -> PerformanceResponse:
        ...
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Failed to get performance: {e!s}"
            )
